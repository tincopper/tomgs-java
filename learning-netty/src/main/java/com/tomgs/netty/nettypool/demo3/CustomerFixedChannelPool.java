 ...
 */
public class CustomerFixedChannelPool extends SimpleChannelPool {
    private static final IllegalStateException FULL_EXCEPTION = ThrowableUtil.unknownStackTrace(
            new IllegalStateException("Too many outstanding acquire operations"),
            CustomerFixedChannelPool.class, "acquire0(...)");
    private static final TimeoutException TIMEOUT_EXCEPTION = ThrowableUtil.unknownStackTrace(
            ...
            CustomerFixedChannelPool.class, "<init>(...)");
    static final IllegalStateException POOL_CLOSED_ON_RELEASE_EXCEPTION = ThrowableUtil.unknownStackTrace(
            ...
            CustomerFixedChannelPool.class, "release(...)");
     ...
     */
    public CustomerFixedChannelPool(Bootstrap bootstrap,
                ...
                case NEW:
                    timeoutTask = new TimeoutTask() {
                        ...
                        public void onTimeout(AcquireTask task) {
                            ...
                            CustomerFixedChannelPool.super.acquire(task.promise);
                        }
                    };
                    break;
                default:
    ...
    private void acquire0(final Promise<CustomerChannel> promise) {
        ...
        if (acquiredChannelCount.get() < maxConnections) {
            ...
            // EventLoop
            Promise<CustomerChannel> p = executor.newPromise();
    ...
    private final class AcquireTask extends AcquireListener {
        final Promise<CustomerChannel> promise;
        final long expireNanoTime = System.nanoTime() + acquireTimeoutNanos;
        ScheduledFuture<?> timeoutFuture;

        AcquireTask(Promise<CustomerChannel> promise) {
            super(promise);
            // We need to create a new promise as we need to ensure the AcquireListener runs in the correct
            // EventLoop.
            this.promise = executor.<CustomerChannel>newPromise().addListener(this);
        }
    }
    ...
    private class AcquireListener implements FutureListener<CustomerChannel> {
        private final Promise<CustomerChannel> originalPromise;
        protected boolean acquired;

        AcquireListener(Promise<CustomerChannel> originalPromise) {
            this.originalPromise = originalPromise;
        }

        @Override
        public void operationComplete(Future<CustomerChannel> future) throws Exception {
            assert executor.inEventLoop();

            if (closed) {
                if (future.isSuccess()) {
                    // Since the pool is closed, we have no choice but to close the channel
                    future.getNow().getChannel().close();
                }
    ...
    private void close0() {
        if (!closed) {
            ...
            GlobalEventExecutor.INSTANCE.execute(new Runnable() {
                @Override
                public void run() {
                    CustomerFixedChannelPool.super.close();
                }
            });
        }
    }
