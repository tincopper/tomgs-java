...
public class SimpleChannelPool implements ChannelPool {
    ...
    @Override
    public final Future<CustomerChannel> acquire() {
        return acquire(bootstrap.config().group().next().<CustomerChannel>newPromise());
    }

    @Override
    public Future<CustomerChannel> acquire(final Promise<CustomerChannel> promise) {
     ...
     */
    private Future<CustomerChannel> acquireHealthyFromPoolOrNew(final Promise<CustomerChannel> promise) {
        try {
    ...
    private void notifyConnect(ChannelFuture future, Promise<CustomerChannel> promise) {
        if (future.isSuccess()) {
            Channel channel = future.channel();
            final CustomerChannel customerChannel = CustomerChannel.builder().channel(channel).build();
            if (!promise.trySuccess(customerChannel)) {
                // Promise was completed in the meantime (like cancelled), just release the channel again
                release(customerChannel);
            }
        } else {
    ...
    private void doHealthCheck(final Channel ch, final Promise<CustomerChannel> promise) {
        assert ch.eventLoop().inEventLoop();
    ...
    private void notifyHealthCheck(Future<Boolean> future, Channel ch, Promise<CustomerChannel> promise) {
        assert ch.eventLoop().inEventLoop();
        ...
        if (future.isSuccess()) {
            if (future.getNow()) {
                try {
                    ...
                    final CustomerChannel customerChannel = CustomerChannel.builder().channel(ch).build();
                    promise.setSuccess(customerChannel);
                } catch (Throwable cause) {
    ...
    @Override
    public final Future<Void> release(CustomerChannel channel) {
        return release(channel, channel.getChannel().eventLoop().<Void>newPromise());
    }

    @Override
    public Future<Void> release(final CustomerChannel channel, final Promise<Void> promise) {
        checkNotNull(channel, "channel");
        checkNotNull(promise, "promise");
        try {
            EventLoop loop = channel.getChannel().eventLoop();
            if (loop.inEventLoop()) {
                doReleaseChannel(channel.getChannel(), promise);
            } else {
                loop.execute(new Runnable() {
                    @Override
                    public void run() {
            ...
            closeAndFail(channel.getChannel(), cause, promise);
        }
        return promise;
    }
